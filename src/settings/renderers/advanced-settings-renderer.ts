...
export class AdvancedSettingsRenderer {
	...
	// Render Context settings section
	...
	private renderContextSettings(containerEl: HTMLElement): void {
		const contextContainer = containerEl.createDiv({ cls: 'llmsider-settings-section-container' });
		
		const contextHeader = contextContainer.createEl('h3', { 
			text: this.i18n.t('settingsPage.contextSettings'),
			cls: 'llmsider-subsection-header'
		});
	...
	private renderOtherAdvancedSettings(containerEl: HTMLElement): void {
		...
		new Setting(otherContainer)
			...
			.setDesc(this.i18n.t('settingsPage.updateNotificationsDesc'))
			.addToggle(toggle => {
				toggle.setValue(this.plugin.settings.updateSettings.notifyUpdates);
				toggle.onChange(async (value) => {
					this.plugin.settings.updateSettings.notifyUpdates = value;
					await this.plugin.saveSettings();
				});
			})
			.addButton(button => {
				button
					.setButtonText(this.i18n.t('settingsPage.checkForUpdates'))
					.setCta()
					.onClick(async () => {
