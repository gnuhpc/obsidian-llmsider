// DuckDuckGo search tools - Enhanced multi-category search - Mastra format
import { z } from 'zod';
...
async function getVqd(query: string): Promise<string> {
  try {
    // Method 1: Try to get VQD from the main page GET request (simpler and less likely to be blocked)
 ...
 */
export const duckduckgoTextSearchTool: MastraTool = createMastraTool({
  category: 'search-web',
  id: 'duckduckgo_text_search',
  description: 'Search the web using DuckDuckGo. REQUIRED: Must provide a search query. Returns structured results with titles, URLs, and snippets. Free, no API key required.',
  
  inputSchema: z.object({
    query: z.string()
      .min(1)
      .describe('REQUIRED: The search query string (e.g., "tesla latest news", "AI technology")'),
    
    region: z.enum(['us-en', 'uk-en', 'de-de', 'fr-fr', 'es-es', 'it-it', 'jp-jp', 'cn-zh', 'tw-zh', 'ru-ru'])
      .optional()
      .default('us-en')
      .describe('Search region (default: us-en)'),
    ...
    try {
      ...
      for (const element of Array.from(resultElements)) {
        ...
        try {
          ...
          // Filter out ads (results with ad_domain)
          const elementHtml = element.outerHTML || '';
          if (elementHtml.includes('ad_domain')) {
            Logger.debug('Skipping ad result:', title);
            continue;
          }

          // 去重检查：跳过已经存在的 URL
          if (seenUrls.has(cleanUrl)) {
            Logger.debug('Skipping duplicate URL:', cleanUrl);
            continue;
          }

          if (title && cleanUrl && body) {
            results.push({ title, url: cleanUrl, body });
            seenUrls.add(cleanUrl); // 记录已添加的 URL
          }
        } catch (error) {
 ...
 */
export const duckduckgoImageSearchTool: MastraTool = createMastraTool({
  category: 'search-web',
  id: 'duckduckgo_image_search',
  description: 'Search for images using DuckDuckGo. REQUIRED: Must provide a search query. Returns structured results with image URLs, thumbnails, and metadata. Free, no API key required.',
  
  inputSchema: z.object({
    query: z.string()
      .min(1)
      .describe('REQUIRED: The image search query (e.g., "mountain landscape", "cat photos")'),
    ...
    size: z.enum(['Small', 'Medium', 'Large', 'Wallpaper'])
      .optional()
      .describe('Image size filter (optional)'),
    
    color: z.enum(['color', 'Monochrome', 'Red', 'Orange', 'Yellow', 'Green', 'Blue', 'Purple', 'Pink', 'Brown', 'Black', 'Gray', 'Teal', 'White'])
      .optional()
      .describe('Color filter (optional)'),
    
    type_image: z.enum(['photo', 'clipart', 'gif', 'transparent', 'line'])
      .optional()
      .describe('Image type filter (optional)'),
    
    layout: z.enum(['Square', 'Tall', 'Wide'])
      .optional()
      .describe('Image layout/aspect ratio (optional)'),
    
    license_image: z.enum(['any', 'Public', 'Share', 'Modify', 'ModifyCommercially'])
      .optional()
      .describe('License type (optional)'),
    ...
    try {
      ...
      if (data.results && Array.isArray(data.results)) {
        for (const item of data.results) {
          ...
          // Filter out ads (results with ad_domain)
          if (item.ad_domain) {
            Logger.debug('Skipping ad result:', item.title);
            continue;
          }
 ...
 */
export const duckduckgoNewsSearchTool: MastraTool = createMastraTool({
  category: 'search-web',
  id: 'duckduckgo_news_search',
  description: 'Search for news articles using DuckDuckGo. REQUIRED: Must provide a search query. Returns structured news results with titles, URLs, dates, and summaries. Free, no API key required.',
  
  inputSchema: z.object({
    query: z.string()
      .min(1)
      .describe('REQUIRED: The news search query (e.g., "tesla earnings", "AI breakthrough")'),
    ...
    timelimit: z.enum(['d', 'w', 'm'])
      .optional()
      .describe('Time limit: d=day, w=week, m=month (optional)'),
 ...
 */
export const duckduckgoVideoSearchTool: MastraTool = createMastraTool({
  category: 'search-web',
  id: 'duckduckgo_video_search',
  description: 'Search for videos using DuckDuckGo. REQUIRED: Must provide a search query. Returns structured video results with titles, URLs, durations, and publishers. Free, no API key required.',
  
  inputSchema: z.object({
    query: z.string()
      .min(1)
      .describe('REQUIRED: The video search query (e.g., "python tutorial", "cooking recipes")'),
    ...
    resolution: z.enum(['high', 'standart'])
      .optional()
      .describe('Video resolution (optional)'),
    
    duration: z.enum(['short', 'medium', 'long'])
      .optional()
      .describe('Video duration (optional)'),
    
    license_videos: z.enum(['creativeCommon', 'youtube'])
      .optional()
      .describe('Video license type (optional)'),
