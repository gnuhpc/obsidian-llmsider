...
/**
 * Path normalization utilities for file operations
 */
export class PathUtils {
	 ...
	 */
	static normalizeFilePath(app: App, path: string): string {
	 ...
	 */
	static normalizeFilePathsInInput(app: App, toolName: string, input: any): any {
	// Tools that work with file paths
	const fileTools = ['view', 'create', 'str_replace', 'insert', 'append', 
	                   'list_directory', 'sed', 'undo_edit'];		if (!fileTools.includes(toolName)) {
			return input;
		}
		...
		if (normalizedInput.path && typeof normalizedInput.path === 'string') {
			...
			normalizedInput.path = this.normalizeFilePath(app, normalizedInput.path);
