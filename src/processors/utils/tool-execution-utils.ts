...
interface ToolManager {
	getToolInfo(toolName: string): Promise<ToolInfo>;
	executeTool(toolName: string, args: unknown): Promise<{ success: boolean; result?: unknown; error?: string }>;
...
export class ToolExecutionUtils {
	...
	static async executeTool(
		toolName: string,
		args: unknown,
		pluginGetToolManager: () => unknown,
		validateToolParameters: (toolName: string, args: unknown) => Promise<{valid: boolean, error?: string, suggestion?: string}>
	): Promise<unknown> {
		...
		const toolManager = pluginGetToolManager() as ToolManager;
		if (!toolManager) {
		...
		try {
			...
			if (!toolInfo.exists) {
				...
				return {
					...
					suggestion: `请只使用以下可用工具之一: ${availableTools.map((t: Tool) => t.name).join(', ')}。请仔细检查工具名称的拼写，确保使用的是实际存在的工具。`
				};
			}
			...
			const missingRequired = requiredParams.filter((param: string) => !(param in (args as object)));
			...
			const unknownParams = Object.keys(args as Record<string, unknown>).filter(param => !availableParams.includes(param));
			...
			return {
				valid: false,
				error: `${(i18n as I18n).t('errors.parameterValidationFailed')}: ${error instanceof Error ? error.message : 'Unknown error'}`
			};
		}
	}
	...
	static async mapInputForTool(
		...
		const parseKeyValuePairs = (input: string): unknown => {
	...
	static cleanToolArguments(
		...
		const cleanedInput = { ...input } as CleanedInput;
