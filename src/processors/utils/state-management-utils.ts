...
import type { Task, TaskStatus } from '../../processors/plan-execute-tracker';
...
export class StateManagementUtils {
	...
	Logger.debug(`===== Updating plan progress =====`);		const planElement = messageContainer.querySelector('.llmsider-plan-phase');
		if (!planElement) {
			Logger.debug('Plan element not found (may still be initializing)');
			return;
		}
		...
		if (progressEl) {
			...
			const getLocalizedToolName = (toolName: string): string => {
				try {
					...
					if (i18nKey?.name) {
						const translated = i18n.t(i18nKey.name);
			...
			if (completedSteps === totalSteps && totalSteps > 0) {
				...
				const currentStep = getCurrentStep() as PlanStep | null;
				if (currentStep) {
					const localizedToolName = getLocalizedToolName(currentStep.tool);
	 ...
	 */
	static getCurrentStep(currentStepIndex: number, planSteps: unknown[]): unknown {
	 ...
	 */
	static areAllStepsCompleted(messageContainer: HTMLElement, planSteps: unknown[]): boolean {
		const completedSteps = messageContainer.querySelectorAll('.plan-todo-item.completed').length;
