...
export const zhOptionsTools = {
	getCurrentTime: {
		...
		optionSseSpotPriceSina: {
			name: '获取上交所期权实时行情',
			description: '获取上交所期权实时行情价格，包括买卖价、成交量、持仓量等信息'
		},
		optionSseUnderlyingSpotPriceSina: {
			name: '获取上交所期权标的物实时行情',
			description: '获取上交所期权标的物实时行情，如50ETF、300ETF、个股等标的资产价格'
		},
		optionSseGreeksSina: {
			name: '获取上交所期权Greeks',
			description: '获取上交所期权的Greeks指标（Delta、Gamma、Theta、Vega、Rho），用于期权风险管理'
		},
		optionSseMinuteSina: {
			name: '获取上交所期权分时数据',
			description: '获取上交所期权分时级别数据，包括价格、成交量、持仓量变化，用于日内分析'
		},
		optionSseDailySina: {
			name: '获取上交所期权日线数据',
			description: '获取上交所期权历史日线数据，包括OHLC、成交量、持仓量、结算价等'
		},
		optionFinanceMinuteSina: {
			name: '获取金融期权分时数据',
			description: '获取金融期权（如中金所50、300、1000指数期权）分时数据'
		},
		optionMinuteEm: {
			name: '获取期权分时数据（东方财富）',
			description: '获取东方财富平台的期权分时数据，覆盖上交所和深交所期权，提供详细的盘中行情'
		},
	optionCffexHs300DailySina: {
		name: '获取中金所沪深300期权日线数据',
		description: '获取中金所沪深300指数期权历史日线数据，包括OHLC、成交量、持仓量、Greeks等'
	},
	optionCffexSz50ListSina: {
		name: '获取中金所上证50期权合约列表',
		description: '获取中金所上证50ETF期权可用合约列表，包括合约代码、行权价、到期日等信息'
	},
	optionCffexHs300ListSina: {
		name: '获取中金所沪深300期权合约列表',
		description: '获取中金所沪深300指数期权可用合约列表，包括合约代码、行权价、到期日等信息'
	},
	optionCffexZz1000ListSina: {
		name: '获取中金所中证1000期权合约列表',
		description: '获取中金所中证1000指数期权可用合约列表，包括合约代码、行权价、到期日等信息'
	},
};