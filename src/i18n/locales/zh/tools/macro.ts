...
getUSNonFarmPayroll: {
		...
		macroChinaCpi: {
			name: '获取中国CPI数据',
			description: '获取中国居民消费价格指数(CPI)数据，反映通货膨胀水平和消费品价格变化'
		},
		macroChinaPpi: {
			name: '获取中国PPI数据',
			description: '获取中国工业生产者出厂价格指数(PPI)数据，反映工业品价格变化和批发通胀趋势'
		},
		macroChinaPmi: {
			name: '获取中国PMI数据',
			description: '获取中国制造业采购经理指数(PMI)数据，包括制造业和服务业PMI，反映经济扩张或收缩状态'
		},
		macroChinaGdp: {
			name: '获取中国GDP数据',
			description: '获取中国国内生产总值(GDP)数据，显示经济增长率和经济规模，提供季度和年度数据'
		},
		macroChinaSocialFinancing: {
			name: '获取中国社会融资数据',
			description: '获取中国社会融资规模数据，反映实体经济获得的融资总量，包括银行贷款、债券、股票等'
		},
		macroChinaMoneySupply: {
			name: '获取中国货币供应量数据',
			description: '获取中国货币供应量M0/M1/M2数据，反映货币流通量和经济中的流动性水平'
		},
		macroChinaForexReserve: {
			name: '获取中国外汇储备数据',
			description: '获取中国外汇储备数据，显示国家储备资产和国际支付能力'
		},
		macroChinaTradeBalance: {
			name: '获取中国贸易差额数据',
			description: '获取中国进出口贸易数据和贸易差额（顺差/逆差），反映对外经济活动状况'
		},
};
