...
export const zhFuturesTools = {
	...
	getRollYieldBar: {
		name: '获取期货展期收益率',
		description: '获取期货展期收益率数据。展期收益率反映了主力合约和次主力合约之间的价差关系，用于判断期货市场的期限结构。'
	},
	getCffexRankTable: {
		name: '获取中金所持仓排名',
		description: '获取中国金融期货交易所（CFFEX）前20会员持仓数据明细，显示股指期货和国债期货的机构持仓情况。'
	},
	getRankTableCzce: {
		name: '获取郑商所持仓排名',
		description: '获取郑州商品交易所（CZCE）会员持仓排名，显示农产品和工业品期货合约的顶级持仓者。'
	},
	getDceRankTable: {
		name: '获取大商所持仓排名',
		description: '获取大连商品交易所（DCE）持仓排名，显示农产品和化工期货合约的顶级会员持仓。'
	},
	// 加密货币市场工具
	cryptoMarketCapRanking: {
		name: '获取加密货币市值排名',
		description: '获取加密货币市值排名。显示主要加密货币按市值排序，包含价格、成交量和市场占有率数据。'
	},
	cryptoBitcoinPriceTrend: {
		name: '获取比特币价格走势',
		description: '获取比特币历史价格走势。提供比特币的OHLCV数据和技术分析指标。'
	},
	cryptoExchangeRanking: {
		name: '获取加密货币交易所排名',
		description: '获取加密货币交易所排名。显示主要交易所按交易量、流动性和支持币种数量排序。'
	},
	cryptoDefiTvl: {
		name: '获取DeFi锁仓总值',
		description: '获取DeFi协议锁仓总值（TVL）数据。显示锁定在去中心化金融协议中的加密资产总量。'
	},
	cryptoNftMarketData: {
		name: '获取NFT市场数据',
		description: '获取NFT（非同质化代币）市场数据。包括交易量、地板价、活跃用户和热门系列。'
	},
	getShfeVWap: {
		name: '获取上期所成交均价',
	...
	getReceiptDate: {
		name: '获取期货仓单有效期',
		description: '获取期货仓单有效期数据，用于了解仓单的到期时间'
	},
	futuresSettlementPriceSgx: {
		name: '获取新加坡交易所期货结算价',
		description: '获取新加坡交易所期货品种每日交易数据'
	},
	futuresZhSpot: {
		name: '获取国内期货实时行情',
	...
	futuresZhRealtime: {
		name: '获取国内期货实时行情',
		description: '获取国内期货指定品种的实时行情数据，包括最新价、涨跌幅、成交量、持仓量等'
	},
	...
	futuresForeignHist: {
		name: '获取外盘期货历史行情',
		description: '获取外盘期货历史行情数据，支持自定义时间范围和周期'
	},
	futuresForeignDetail: {
		name: '获取外盘期货合约详情',
		description: '获取外盘期货合约详细信息，包括合约规格、交易时间、交割规则等'
	},
	futuresZhMinuteSina: {
		name: '获取国内期货分时数据',
		description: '获取国内期货分时数据，用于日内交易分析和走势研判'
	},
};