...
export class MastraAgent {
	...
	private async executeWithFallback(options: AgentExecuteOptions): Promise<void> {
		...
		// Agent 模式不需要生成 final answer，所有执行结果已经通过工具执行显示
	...
	private async generatePlan(messages: ChatMessage[]): Promise<AgentPlan> {
		...
		try {
			...
			await this.provider.sendStreamingMessage(
				...
				(chunk) => {
					chunkCount++;
					// Logger.debug('[MastraAgent] 📦 Received chunk #' + chunkCount + ':', {
					...
					if (chunk.metadata) {
						// Logger.warn('[MastraAgent] ⚠️ Chunk has metadata:', chunk.metadata);
					}
				}
			);
			Logger.debug('[MastraAgent] ✅ sendStreamingMessage completed');
			...
			Logger.error('[MastraAgent] ❌ API call CAUGHT ERROR in inner try-catch');
		...
		if (apiError) {
			Logger.error('[MastraAgent] ⚠️ API ERROR DETECTED - Re-throwing to preserve original error');
		...
		if (!planText || planText.trim().length === 0) {
			Logger.error('[MastraAgent] ❌ planText is empty, throwing new error');
			throw new Error('No plan content received from LLM');
		}
		
		Logger.debug('[MastraAgent] ✅ planText validated, proceeding to parse...');
	...
	private async getAvailableToolsDescription(): Promise<string> {
		...
		if (outputSchema) {
					...
					Object.entries(jsonSchema.properties).forEach(([fieldName, fieldSchema]: [string, any]) => {
						...
						if (fieldType === 'array' && fieldSchema.items?.type === 'object' && fieldSchema.items?.properties) {
							...
							Object.entries(fieldSchema.items.properties).forEach(([itemFieldName, itemFieldSchema]: [string, any]) => {
								...
								outputInfo += `      • ${itemFieldName}: ${itemFieldType}${itemFieldDesc ? ' - ' + itemFieldDesc : ''}\n`;
							});
						}
					});
			...
			outputInfo += `\n💡 Access in next step: {{stepX.fieldName}} or {{stepX}}\n`;
		}
	...
	private async buildPlanningPrompt(query: string, context: ChatMessage[]): Promise<string> {
		...
		if (attachedFiles.length > 0) {
			const fileList = attachedFiles.map(f => `- ${f.name}${f.path !== f.name ? ` (路径: ${f.path})` : ''}`).join('\n');
			const examplePath = attachedFiles[0].path || attachedFiles[0].name;
			
			contextInfo = `用户已附加以下文件（完整内容已在系统提示词中）：
${fileList}

**重要规则**：
1. 当用户说"根据这个"、"基于这个"、"参考这个"时，指的是上述附加文件
2. 附加文件的完整内容已包含在系统提示词中，你可以直接使用
3. **禁止**将完整文件内容作为搜索查询（会因过长而失败）

**关键规则 - 何时需要总结步骤**：
⚠️ **仅当满足以下条件时，才需要先用 generate_content 总结**：
- 用户要求基于附加文件内容进行**网络搜索**
- 例如："根据这个笔记搜索XXX"、"基于这篇文章查找资料"

在这种情况下，正确流程是：
→ step1: generate_content(task="提取文件中关于XXX的核心信息，生成2-3个搜索关键词（20字以内）")
→ step2: duckduckgo_text_search(query="{{step1.content}}")
→ step3: 处理搜索结果...

**无需总结的情况**（直接使用文件内容）：
- 用户只要求"根据这个文件生成XXX"（无搜索需求）
  → step1: generate_content(task="基于附加文件内容生成XXX")
- 用户要求"分析这个文件"、"总结这篇文章"
  → step1: generate_content(task="分析/总结文件内容")

**仅在以下情况使用 view 工具**：
- 需要查看**未附加**的其他文件时（路径示例："${examplePath}"）`;
	...
	private inferDependencies(steps: AgentStep[]): void {
		...
		steps.forEach(step => {
			...
			if (inferredDeps.size > 0) {
				...
				Logger.debug(`[MastraAgent] ✓ Inferred dependencies for ${step.id}:`, step.dependencies);
			} else {
				step.dependencies = [];
				Logger.debug(`[MastraAgent] ○ No dependencies inferred for ${step.id} (independent)`);
			}
		});
	...
	private parsePlanFromResponse(response: string): AgentPlan {
		try {
			...
			if (stepsWithDeps.length === 0 && steps.length > 1) {
				Logger.warn('[MastraAgent] ⚠️ No dependencies detected in plan! Auto-inferring from placeholders...');
...
- Ready to use without modifications`;
				...
				if (response.results && Array.isArray(response.results)) {
					const contents = response.results
						.map((r: any, idx: number) => {
							const title = r.title || r.name || `搜索结果 ${idx + 1}`;
	 ...
	 * Removes all processing artifacts like "抓取成功"、"内容长度"、"单词数"、"抓取耗时" etc.
	 */
	private cleanTextContent(text: string): string {
		...
		return text
			...
			.replace(/【.*?摘要】/g, '')
			.replace(/【.*?结果】/g, '')
			.replace(/^#+\s*(Web Content Fetch Results|Fetch Results|搜索结果|抓取结果).*$/gm, '')
			
			// Remove success/failure status indicators
			.replace(/^[✅❌*-]\s*\*\*?(Status|状态|抓取成功|成功|失败)\*\*?[:：].*$/gm, '')
			.replace(/^\*\*?(抓取成功|抓取失败|成功|失败)\*\*?[:：].*$/gm, '')
			
			// Remove statistics and metrics (Chinese and English)
			.replace(/^\*\*?(Total|总计|总数).*?\*\*?[:：].*$/gm, '')
			.replace(/^\*\*?(Success|Successful|成功数|成功).*?\*\*?[:：].*$/gm, '')
			.replace(/^\*\*?(Failed|Failure|失败数|失败).*?\*\*?[:：].*$/gm, '')
			.replace(/^\*\*?(Content Length|内容长度).*?\*\*?[:：].*$/gm, '')
			.replace(/^\*\*?(Word Count|单词数|词数).*?\*\*?[:：].*$/gm, '')
			.replace(/^\*\*?(Duration|耗时|抓取耗时|用时).*?\*\*?[:：].*$/gm, '')
			.replace(/^\*\*?(Method|方法|提取方法|抓取方法).*?\*\*?[:：].*$/gm, '')
			
			// Remove bullet points with metadata
			.replace(/^[*-]\s+(标题|内容长度|单词数|抓取耗时|抓取成功|成功|失败|方法)[:：].*$/gm, '')
			
			// Remove numbered list items with metadata
			.replace(/^\d+\.\s+\*\*?(标题|内容长度|单词数|抓取耗时|抓取成功|成功|失败)\*\*?[:：].*$/gm, '')
			...
			.replace(/^###?\s*(Content Preview|内容预览).*$/gm, '')
	...
	// Agent 模式不生成 final answer - 已删除 generateFinalAnswer 方法
	// 所有执行结果通过工具执行过程直接显示，无需额外生成总结性回答
