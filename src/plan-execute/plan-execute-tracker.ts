...
export class PlanExecuteTracker {
	...
	private scrollToTask(taskId: string): void {
		...
		setTimeout(() => {
			Logger.debug('scrollToTask called for:', taskId);
			...
			Logger.debug('Task index:', taskIndex, 'Total elements:', taskElements.length);
			...
			if (taskIndex >= 0 && taskIndex < taskElements.length) {
				...
				Logger.debug('Scroll container found for auto-scroll:', !!scrollContainer);
				
				if (scrollContainer && taskElement) {
					Logger.debug('Scrolling to task element');
	...
	private renderHeader(container: HTMLElement): void {
		...
		if (this.props.isExecuting) {
			...
			completedEl.createEl('span', { text: `${completedCount} 成功` });
			...
			failedEl.createEl('span', { text: `${errorCount} 失败` });
		}
	...
	private renderTask(container: HTMLElement, task: Task, index: number): void {
		Logger.debug(`===== Rendering Task ${index + 1} =====`);
		Logger.debug(`Task ID: ${task.id}`);
		Logger.debug(`Task Status: ${task.status}`);
		Logger.debug(`Task Error: ${task.error}`);
		Logger.debug(`Has ToolCalls: ${task.toolCalls?.length || 0}`);
		Logger.debug(`Is Historical Plan: ${!this.props.isExecuting}`);
		...
		if (mainToolCall) {
			Logger.debug(`MainToolCall Error: ${mainToolCall.error}`);
		}
		...
		Logger.debug(`===== Task ${task.id} Styling Debug =====`);
		Logger.debug(`Status: ${task.status}`);
		Logger.debug(`Config Border Class: ${config.border}`);
		Logger.debug(`Config BG Class: ${config.bg}`);
		Logger.debug(`Final className: "${taskItem.className}"`);
		Logger.debug(`Element classList:`, Array.from(taskItem.classList));
		...
		// Only show status icon for executing plans
		if (!isHistorical) {
