...
export class PlanCheckProcessor {
	 ...
	 */
	async validatePlanSteps(planSteps: any[], conversationMessages: ChatMessage[]): Promise<{
		isValid: boolean;
		validatedSteps?: any[];
		issues?: Array<{
			stepId: string;
		...
		try {
			...
			if (validationResults.issues.length > 0) {
				...
				if (correctionResult.success) {
					...
					setTimeout(() => this.hideValidationIndicator(), 800);
	 ...
	 */
	private async getToolSchemas(): Promise<Map<string, any>> {
	 ...
	 */
	private async analyzeStepParameters(planSteps: any[], toolSchemas: Map<string, any>): Promise<{
		issues: Array<{
			stepId: string;
			toolName: string;
			issue: string;
	 ...
	 */
	private validateParameters(input: any, schema: any): {
		valid: boolean;
		error?: string;
		suggestion?: string;
	} {
 ...
 */
private checkForUnreplacedPlaceholders(input: any): {
valid: boolean;
error?: string;
suggestion?: string;
} {
...
return {
valid: false,
error: `参数包含未替换的步骤占位符: ${stepMatches.join(', ')}`,
suggestion: '确保前面的步骤已执行完成并提供了所需的数据，或替换为具体的参数值'
};
}
