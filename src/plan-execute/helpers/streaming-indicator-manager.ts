...
export class StreamingIndicatorManager {
	...
	createToolIndicator(): HTMLElement {
		...
		(cardContainer as any).toolCardData = {
			toolName: '',
		...
		(cardContainer as any).toolCardInstance = null;
	...
	updateToolIndicator(
		...
		// Get stored data
		const cardData = (this.currentToolIndicator as any).toolCardData;
			...
			case 'error':
				...
				cardData.onRegenerateAndRetry = () => {
					...
					if (pendingToolFailure) {
						...
						Logger.error('pendingToolFailure is null!');
					}
				};
		...
		// Store card instance
		(this.currentToolIndicator as any).toolCardInstance = cardInstance;
